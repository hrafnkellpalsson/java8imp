...
//        File[] dirs = file.listFiles(f -> f.isDirectory());
     ...
     * forEachIf(Consumer<T> action, Predicate<T> filter) that applies action to each element for which filter returns
     * true. How could you use it?
     */
    public void ex9() {
    ...
    /**
     * Go through the method of the Collections class. If you were king for a day, into which interface would you place
    ...
    /**
     * In the past, you were told that it's bad form to add method to an interface because it would break existing
     * code. Now you are told that it's okay to add new methods, provided you also supply a default implementation. How
     * safe is that? Describe a scenario where the new stream method of the Collection interface causes legacy code to
     * fail compilation. What about binary compatibility? Will legacy code from a JAR file still run?
     */
    public void ex12() {
