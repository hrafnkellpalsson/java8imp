...
// Bloch: There is a duality between type parameters and wildcards, and many method can be declared using one or the other.
// Type 2
// Bounded generic type: List<E extends String>
// A special subtype is recursively bounded generic type. The most common is the following (and the type bound may be
// read as "for every type E that can be compared to itself"): List<E extends Comparable<E>>
// Type 3
...
// Bloch rule: If a type parameter appears only once in a method declaration, replace it with a wildcard.
// representing a set that can contain only objects of some unknown type.
// Type 4
// Bounded wildcard type: List<? extends String> or List<? super String> - For covariance and contravariance, see
// classes MyStack and MyStackTest.
public class Generics {
  ...
  // Knowing how to create a generic array is useful for creating generic collections backed by an array.
  public static <E> E[] createGenericArray(E[] arr) {
    // Creating a generic array by new-ing it is not allowed (compare with C# ": where new" construct)
    // return new E[];
    // We can however create a generic array by casting.
    Object[] brr = Arrays.copyOf(arr, arr.length);
    // The elements of this array will only contain E instances since the input parameter is of type E[].
  ...
  public static void usingWildcardsInCode(Set<?> s) {
  ...
  // Method signature could be improved, see method comparableGenerics, but the point here is to show a recursive bound.
  public static <E extends Comparable<E>> void recursivelyBoundedGeneric(E elem) {
    System.out.println("elem = [" + elem + "]");
  }

  // Comparable is a consumer of E elements and so by the PECS rule we should use <? super E> in preference to <E>.
  // Same goes for Comparator.
  public static <E extends Comparable<? super E>> int comparableGenerics(E e1, E e2) {
    return e1.compareTo(e2);
  }

  // On duality between type parameters and wildcards. A method that swaps two elements in a list can be declared in
  // either of following ways
  // public static void swap(List<?> list, int i, int j) or
  // public static <E> void swap(List<E> list, int i, int j)
  // Bloch deems first one as slightly better because there is no type parameter to worry about, but it's also
  // slightly harder to implement, see Bloch. Also, this declaration falls under Bloch rule about a type parameter
  // that appears only once.

  public void usingRawTypes(Object o) {
    // There are only two exceptions to using raw types in new code.
    // First.
    // You must use raw types in class literals. List<String>.class is forbidden so we must use List.class (note
    // that String.class, String[].class and int.class are all legal).
    // Second.
    // Concerns instanceof operator. Because of type erasure at runtime it is illegal to use instanceof operator
    // on parameterized types except on unbounded wildcard types, because that's the obscure exception where type
    // erasure does not happen!
    // if (o instanceof Set<String>) - Illegal!
    // if (o instanceof Set<?>) - Bloch considers the "<?>" to be unnecessary noise here and prefers dropping it
    if (o instanceof Set) {
      Set<?> s = (Set<?>) o;
    }
  }
